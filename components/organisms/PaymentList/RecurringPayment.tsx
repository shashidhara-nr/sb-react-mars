...
import { RECURRING_PAYMENTS_TABLE_COLUMNS, RECURRING_PAYMENTS_TABLE_HEAD_CELLS } from './constant';
import { mockPayments } from '@lib/mock/mockPayments';
import RecurringPaymentFilterDialog from './RecurringPaymentFilterDialog';
...
const RecurringPayment = () => {
  ...
  const allTableHeadCells = useMemo(
    () =>
      RECURRING_PAYMENTS_TABLE_HEAD_CELLS.map(cell => ({
        ...cell,
  ...
  // Memoize table data
  const recurringPaymentsTableData = useMemo(
    () => ({
      columns: RECURRING_PAYMENTS_TABLE_COLUMNS,
  ...
  const handleTabChange = useCallback((_event: React.SyntheticEvent, newValue: string) => {
    ...
    setDownloadDialogOpen(false);
    setSelectedRows([]);
    setCurrentTableData(recurringPaymentsTableData);
  }, [recurringPaymentsTableData]);

  const statusTabs = useMemo(
    () => [
      { label: t('awaitingApproval'), value: '0' },
      { label: t('processing'), value: '1' },
      { label: t('active'), value: '2' },
      { label: t('expired'), value: '3' },
      { label: t('declined'), value: '4' },
    ],
    [t],
  );

  const [currentTableData, setCurrentTableData] = useState<any>(recurringPaymentsTableData);
    ...
    <section className={`${styles.tabContent} ${styles.accountsAndBalancesContainer}`}>
      <Grid size={12} className={styles.searchRow}>
        <TextField
          fullWidth
          variant="outlined"
          placeholder={t('searchRecurringPayments')}
          value={searchText}
      ...
      <section className={styles.tableContainer}>
        ...
        <DownloadDebtorDialog
          ...
          title={`${t('download')} ${t('recurringPayments').toLowerCase()}`}
        />
      </section>
  </section>
  );
