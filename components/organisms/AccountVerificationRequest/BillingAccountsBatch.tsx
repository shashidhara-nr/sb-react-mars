...
import BatchAccountVerificationRequestFilter from './BatchAccountsFilterDialog';
...
const BillingAccountsBatch = ({edit = true}: {edit: boolean}) => {
  ...
  const [billingAccountInfo, setBillingAccountInfo] = useState({});
      ...
      <Box className={styles.batchContent}>
        ...
        <Box className={styles.batchInnerContent}>
          ...
          <Box className={styles.batchInnerFooter}>
            {addedAccountList.length !== 0 && (
              <CustomPagination
                rows={addedAccountList}
                page={page}
                rowsPerPage={perPage}
